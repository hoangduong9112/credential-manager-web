...
const Sidebar: FC = () => {
	...
	const pageRendering: PageRender | undefined = useSelector((state: RootState) => state.pageRendering.currentPageRendering);
	...
	const handleClickSearchFamily = async (event: any) => {
		switch (pageRendering) {
			case PageRender.LIST_CREDENTIAL:
				{
					const res = await instance.get(`congDan/search?lastname=${formSearch.ownerName}&pageSize=4&page=1`);
			...
			case PageRender.LIST_FAMILY:
				{
					const res = await instance.get(`hoKhau/search?cccd=${formSearch.cccd}`);
					if (res.data.status) {
			...
			case PageRender.LIST_EVENT:
				{
					const res = await instance.get(`dongGop/search?name=${formSearch.eventName}`);
					console.log(res.data);
					if (res.data.status) {
				...
				{!!pageRendering && (
					<Form className="">
						<Row className="align-items-center d-flex justify-content-center ml-1">
							<FormControl type="search" placeholder={generateSearchSuggestion(pageRendering)} className="search-field me-2" aria-label="Search" onChange={handleChangeSearchFamily} />
							<Col xs="auto">
